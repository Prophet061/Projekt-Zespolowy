...
$collected_data = new Array();
function collect($current, $collected, $series, $max){
  if($current > 0){
    $chart_values = new Array();

    $.each(Object.keys($collected), function($i, $symbol){
      $color = dynamicColors();
  ...
  $.ajax({
    ...
    $.each($data.rates, function(date, e) {
      ...
      $.each(e, function($symbol, $v) {
        if(!$collected[$symbol]) $collected[$symbol] = new Array();

        $v = 1/$v;

        $collected[$symbol].push($v);

        if($v > $max) $max = $v;
      })
    })

    collect(++$current, $collected, $series, 0);
  }).fail(function($r) {
    d($r.responseText);
  });
}
...
function fetchData() {
  ...
  collect($duration, new Array(), new Array(), 0);
}
...
function savePreset() {
  ...
  $saves = localStorage.getItem("shortcuts-changes") ? JSON.parse(localStorage.getItem("shortcuts-changes")) : [];
  $saves.unshift($save);

  localStorage.setItem("shortcuts-changes", JSON.stringify($saves));
  loadPresets();
}
...
function loadPresets() {
  ...
  $.each($saves, function(i, e) {
    ...
    $url = "/site/changes?base={0}&duration={1}&currencies={2}";
...
function resizeChart() {
  $w = $("#chart").width();
  $h = $("#chart").height();
  // if(typeof(chart) != "undefined") chart.resize($w, $h);
}
...
$(document).ready(function() {
  ...
  $duration = getUrlParameter("duration") ? getUrlParameter("duration") : -4;
