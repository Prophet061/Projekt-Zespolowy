...
function fetchData() {
  ...
  $url = $url.f($date_start, $date_end, "PLN", $symbols);
...
function savePreset() {
  ...
  $saves = localStorage.getItem("shortcuts-zapps") ? JSON.parse(localStorage.getItem("shortcuts-zapps")) : [];
  $saves.unshift($save);

  localStorage.setItem("shortcuts-zapps", JSON.stringify($saves));
  loadPresets();
}
...
function loadPresets() {
  ...
  $.each($saves, function(i, e) {
    ...
    $url = "/site/zapps?base={0}&duration={1}&currencies={2}";
...
$(document).ready(function() {
  $base = getUrlParameter("base") ? getUrlParameter("base") : "ŻAPPS";
  $base_nice = $("#base").find("option[value='" + $base + "']").text();
